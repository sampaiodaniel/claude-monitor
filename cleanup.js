// Auto-run: deduplicate all entries for the active account (keep highest session% per 3h window)
chrome.storage.local.get(['accounts', 'activeAccountId'], (state) => {
  const logEl = document.getElementById('log');
  const activeId = state.activeAccountId;

  if (!activeId) {
    logEl.textContent = 'Nenhuma conta ativa. Faça login em claude.ai primeiro.';
    return;
  }

  const logKey = `account:${activeId}:usageLog`;
  chrome.storage.local.get({ [logKey]: [] }, (result) => {
    const log = result[logKey];
    const before = log.length;

    if (before === 0) {
      const accounts = state.accounts || {};
      const name = accounts[activeId]?.customLabel || accounts[activeId]?.orgName || activeId.slice(0, 8);
      logEl.textContent = `Log vazio para conta "${name}".`;
      return;
    }
    ...
    chrome.storage.local.set({ [logKey]: cleaned }, () => {
      const accounts = state.accounts || {};
      const name = accounts[activeId]?.customLabel || accounts[activeId]?.orgName || activeId.slice(0, 8);
      logEl.textContent =
        `Conta: ${name}\n` +
